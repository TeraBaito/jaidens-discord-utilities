...
module.exports = {
    ...
    run: async(bot, message, args) => {
        ...
        if (!args[0]) return message.channel.send('Please provide a valid user to mute')
        ...
        if (!message.guild.me.permissions.has(FLAGS.MANAGE_ROLES)) return message.channel.send('I don\'t have permissions to mute users, please enable the "Manage Roles" permission')
            .then(m => setTimeout(() => { m.delete(); }, 5000));
    
        // Can't find member
        if (!toMute) return message.channel.send('Couldn\'t find that member, try again')
            .then(m => setTimeout(() => { m.delete(); }, 5000));
